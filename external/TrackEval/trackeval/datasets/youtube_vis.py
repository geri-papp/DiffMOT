...
class YouTubeVIS(_BaseDataset):
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        for t in range(num_timesteps):
            ...
            raw_data["ids"][t] = np.atleast_1d([track["id"] for track in tracks if track["segmentations"][t]]).astype(
                int
            )
        ...
        raw_data["classes_to_tracks"] = {
            cls: [{i: track["segmentations"][i] for i in range(len(track["segmentations"]))} for track in tracks]
